...
coco_names_path = '../datasets/coco_Xray.names'
def get_class_names(coco_names_path):
    class_names = {}
    with open(coco_names_path, 'r') as file:
        for line in file:
            line = line.strip()  # 去除每行两端的空白字符（如换行符、空格等）
            parts = line.split(':')  # 以冒号分割，得到英文和中文部分
            if len(parts) == 2:
                english_name = parts[0].strip()
                chinese_name = parts[1].strip()
                class_names[english_name] = chinese_name
    return class_names
class_names = list(get_class_names(coco_names_path).items())

# Create a list of colors for each class where each color is a tuple of 3 integer values
rng = np.random.default_rng(3)
colors = rng.uniform(0, 255, size=(len(class_names), 3))

# tcp_server receive image data from tcp_client
def receive_data(sock, length):
    data = b''
    while len(data) < length:
        try:
            received = sock.recv(length - len(data))
            if not received:
                break
            data += received
        except socket.error as e:
            logging.error(f"接收数据时出现错误: {e}")
            break
    return data

# tcp_server send image data to tcp_client
...
def draw_detections(image, boxes, scores, class_ids, mask_alpha=0.3, mask_maps=None):
    ...
    for box, score, class_id in zip(boxes, scores, class_ids):
        ...
        print(x1,y1,x2,y2,class_id,class_names[class_id])
        # Draw rectangle
